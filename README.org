...
- ~.md~ → ~gfm-mode~ （GitHub-Flavoured，支持任务列表、围栏代码块、自动链接等）
- ~.markdown~ → ~markdown-mode~ （传统 CommonMark）
- 围栏代码块按其声明的语言原生着色（ ~markdown-fontify-code-blocks-natively~ ）
- 行号 + ~visual-line-mode~ 软换行（中文也能正确按字符断）
- 数学公式语法默认开启 （ ~$x^2$~ ）
- 保存时由 apheleia 调 ~prettier~ 自动格式化（如果系统里有 ~prettier~ ；没有就跳过）
...
| 升 / 降级标题              | ~M-←~ / ~M-→~                           |
| 实时预览                   | ~C-c C-c p~                             |
| 切换斜体 / 粗体 / 行内代码 | ~C-c C-s i~ / ~C-c C-s b~ / ~C-c C-s C~ |
| 插入链接                   | ~C-c C-l~                               |
| 插入表格                   | ~C-c C-t t~                             |
